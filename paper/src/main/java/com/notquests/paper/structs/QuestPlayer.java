...
import com.notquests.paper.commands.NotQuestColors;
...
public class QuestPlayer {
    ...
    public void giveReward(final Quest quest) {
        ...
        for (Action action : quest.getRewards()) {
            main.getActionRunner().executeActionWithConditions(action, this, player, true, quest);
    ...
    public void forceActiveQuestCompleted(final ActiveQuest activeQuest) {
        ...
        for (final ActiveQuest activeQuest2 : activeQuests) {
            for (final ActiveObjective objective : activeQuest2.getActiveObjectives()) {
                OtherQuest.addProgressIfTargets(objective, activeQuest.getQuest());
            }
        }
        ...
        for (final ActiveQuest activeQuest : getActiveQuests()) {
            for (final ActiveObjective activeObjective : activeQuest.getActiveObjectives()) {
                if (ConditionWatch.isConditionWatch(activeObjective.getObjective())) {
                    if (ConditionWatch.checkOnlyWhenCorrespondingVariableValueChanged(activeObjective.getObjective()) || !activeObjective.isUnlocked()) {
                        continue;
                    }
                    ConditionWatch.updateProgressIfFulfilled(activeObjective, this);
                    ...
                    if (NumberVariable.checkOnlyWhenCorrespondingVariableValueChanged(activeObjective.getObjective())
