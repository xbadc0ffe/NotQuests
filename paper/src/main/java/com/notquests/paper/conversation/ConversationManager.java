...
import com.notquests.paper.conversation.interactionhandlers.ConversationInteractionHandler;
import com.notquests.paper.conversation.interactionhandlers.SendClickableText;
import com.notquests.paper.managers.data.Category;
import com.notquests.paper.managers.npc.NQNPC;
import com.notquests.paper.structs.QuestPlayer;
import com.notquests.paper.actions.*;
      ...
      for (String conditionString : allConditionsString) {
        ...
        if (conditionString.toLowerCase(Locale.ROOT).startsWith("condition ")) {
          ...
          final Condition foundCondition =
              main.getSavedConditions().getCondition(strippedCondition);
          ...
          final String conditionTypeIdentifier = singleLineConditionStringArguments.get(0);

          Condition condition = main.getConditionCatalog().createCondition(conditionTypeIdentifier);
          if (condition != null) {
            singleLineConditionStringArguments.remove(0);
          } else {
            for (String variableString : main.getVariableCatalog().getVariableIdentifiers()) {
              ...
              Variable<?> variable =
                  main.getVariableCatalog().getVariableFromString(variableString);
              if (variable == null) {
                continue;
              }
              final String variableConditionType = switch (variable.getVariableDataType()) {
                case NUMBER -> "Number";
                case STRING -> "String";
              ...
              condition = main.getConditionCatalog().createCondition(variableConditionType);
              if (condition == null) {
                continue;
              }
              try {
          ...
          try {
            ...
            main.getLogManager()
                .warn(
                    "Unable to create conversation line condition: "
    ...
    if (allActionsString != null && !allActionsString.isEmpty()) {
      ...
      for (String actionString : allActionsString) {
        ...
        if (actionString.toLowerCase(Locale.ROOT).startsWith("action ")) {
          ...
          final String actionTypeIdentifier = singleLineActionStringArguments.get(0);

          Action action = main.getActionCatalog().createAction(actionTypeIdentifier);
          if (action != null) {
            ...
            for (String variableString : main.getVariableCatalog().getVariableIdentifiers()) {
              ...
              final String variableActionType = switch (variable.getVariableDataType()) {
                case NUMBER -> "Number";
                case STRING -> "String";
                case BOOLEAN -> "Boolean";
                case LIST -> "List";
              ...
              action = main.getActionCatalog().createAction(variableActionType);
              if (action == null) {
                continue;
              }
              try {
          ...
          try {
            ...
            main.getLogManager()
                .warn(
                    "Unable to create conversation line action: "
