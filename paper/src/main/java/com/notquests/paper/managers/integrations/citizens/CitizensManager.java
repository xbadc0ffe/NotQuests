package com.notquests.paper.managers.integrations.citizens;
...
public class CitizensManager {
  ...
  public void handleEscortObjective(final ActiveObjective activeObjective) {
    final Integer npcToEscortId = com.notquests.paper.builtin.objectives.EscortNPC.npcToEscortId(activeObjective);
  ...
  public void startEscortObjective(
      final int npcToEscortID,
    ...
    if (npcToEscort != null && destinationNPC != null) {
      ...
      if (!Bukkit.isPrimaryThread()) {
          ...
          Bukkit.getScheduler()
              .runTask(
                  ...
                  () -> {
                    ...
                    startEscortObjectiveSynchronous(
                        npcToEscort, destinationNPC, followTrait, activeObjectiveHolder, spawnLocation);
                  });
        } else {
        ...
        startEscortObjectiveSynchronous(
            npcToEscort, destinationNPC, followerTrait, activeObjectiveHolder, spawnLocation);
      }
  ...
  private void startEscortObjectiveSynchronous(
      final NPC npcToEscort,
      final NPC destinationNPC,
      final FollowTrait followerTrait,
      final ActiveObjectiveHolder activeObjectiveHolder,
      final Location configuredSpawnLocation) {
    final Player player = Bukkit.getPlayer(activeObjectiveHolder.getQuestPlayer().getUniqueId());
    if (player != null) {
      final Location spawnLocation =
          configuredSpawnLocation != null
